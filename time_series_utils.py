...
def series_crossunder(series1, series2):
    return (series1 < series2) & (series1.shift(1) > series2.shift(1))


def rr_order_block(close, high, low, risk=0.1, ratio=1, lead=15, ob_type = 'buy'):
    """
    Calculate y series for which risk reward results.
    Shifting y series from 1 to lead bars to the left.
    0: undefined
    1: loss
    2: win
    """
    ...
    if ob_type == 'buy':
        sl = close - risk * (close)
        tp = close + risk * ratio * (close)

        for i in range(lead):
            y = np.where((sl > low.shift(-i))&(y==0), 1, y)
            y = np.where((tp < high.shift(-i))&(y==0), 2, y)
    elif ob_type == 'sell':
        sl = close + risk * (close)
        tp = close - risk * ratio * (close)

        for i in range(lead):
            y = np.where((sl < high.shift(-i))&(y==0), 1, y)
            y = np.where((tp > low.shift(-i))&(y==0), 2, y)
    
    return pd.Series(data=y, index=close.index)